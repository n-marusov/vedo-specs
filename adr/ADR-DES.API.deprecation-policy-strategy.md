...
Enterprise-клиенты с активным льготным периодом исключаются из снижения лимитов.

### 6. Льготный период (Enterprise)
...
| Без льготного периода для Enterprise | Критические интеграции Enterprise требуют больше времени на миграцию |
...
4. **Гибкость:** Льготный период для Enterprise, снижение лимитов для стимулирования миграции
5. **Минимальное внедрение:** Прослойка в существующем API Gateway, не требует нового сервиса
...
Автоматизация через прослойку и конфигурацию (Sunset header добавляется декларативно)
...
- [ ] Льготный период: API + БД-таблица
