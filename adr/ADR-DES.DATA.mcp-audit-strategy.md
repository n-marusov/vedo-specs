...
- Восстановить цепочку доказательств (непрерывность контроля)
- Соответствовать нормативным требованиям (GDPR/152-ФЗ)
...
- Полные MCP-ответы сохраняются с префиксом `mcp-audit/` и сроком хранения 90 дней через политику жизненного цикла S3
...
### 6. Срок хранения (автоматическое удаление)

| Тип данных | Срок | Механизм |
|------------|------|----------|
| **Полные ответы (S3)** | 90 дней | Политика жизненного цикла S3 |
...
### 8. Аудит доступа к audit-логам (непрерывность контроля)
...
5. **Автоматический срок хранения:** политика жизненного цикла S3 + pg_partman — без ручного вмешательства.
...
3. **Связность:** Асинхронная запись в S3 (после ответа клиенту); плавная деградация при недоступности S3 (логирование без полного ответа).
...
3. **Рост объёма логов:** Автоматический срок хранения через pg_partman и политику жизненного цикла S3.
