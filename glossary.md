...
Язык запросов к API, позволяющий клиенту явно запрашивать нужную структуру данных. В VEDO Core используется как основной API фронтенда для навигации по графу онтологии: подгрузки порций графа, cursor pagination, вложенных запросов и работы с Apollo Client. Динамические свойства онтологии возвращаются через контейнеры `propertyValues`, `outgoingEdges` и `incomingEdges`, а не как статические поля схемы.
...
В рамках VEDO Core — изолированное пространство данных, содержащее одну или несколько онтологий (проектов). Служит единицей адресации при работе через API и разграничения прав доступа для команд.

#### Группа (Group)
`group`

Контейнер для объединения проектов (онтологий) и вложенных групп, соответствующий организационной структуре команд, департаментов или направлений. Поддерживает иерархию: группа может содержать другие группы. Членство и права доступа наследуются по иерархии: роль, назначенная на группу, применяется ко всем вложенным группам и проектам. Аналог Group в GitLab.

#### Проект (Project)
`project`

Базовая единица работы, доступа и независимого версионирования в VEDO Core. Проект — это среда для разработки конкретной онтологии. Каждый проект имеет собственный Git-подобный репозиторий (история коммитов, ветки, Merge Request'ы), независимый набор участников с ролями и изолированное пространство данных. Аналог Project в GitLab.
...
#### Шаблон онтологии (Ontology Template)
`ontologyTemplate`

Готовая, структурированная, документированная и валидная OWL-онтология, предназначенная для использования в качестве отправной точки при создании новой онтологии. В отличие от обычной онтологии, шаблон содержит минимальный ABox (1–2 демонстрационных индивида), обязательную документацию (описание предметной области, карту классов, примеры запросов) и метаданные (название, описание, версия в формате semver, автор, теги, категория, `usage_count`). Включает TBox с классами, `ObjectProperty`, `DatatypeProperty`, иерархиями, аксиомами (`rdfs:subClassOf`, `rdfs:domain`, `rdfs:range`, `owl:inverseOf`, OWL DL-конструкции) и аннотации (`rdfs:label`, `rdfs:comment`). Шаблон хранится как OWL-онтология в формате Turtle/RDF XML с сопроводительным файлом метаданных. В VEDO Core шаблоны подвергаются семантическому версионированию, пересмотру каждые 6 месяцев и удалению при `usage_count` < 5 за период пересмотра.
