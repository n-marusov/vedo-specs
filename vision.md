...
|---|---|---|---|
| **1** | **F13: Социальный хаб (GitHub-стиль)** | Форки онтологий (F13.1 — **MVP** через `POST /api/v1/projects/{id}/fork`), звёзды, issues, pull requests, профили авторов с рейтингом и графом вклада, DOI для цитирования, спонсорство, партнёрские комиссии (post-MVP). | Онтологии — живые активы с комьюнити, академическим признанием и экономическими стимулами. Fork механизм в MVP обеспечивает базовый социальный сценарий: пользователи форкают демо-проекты и публичные онтологии. |
...
| F3.4 | История | Просмотр лога коммитов, переход к конкретному коммиту (checkout). Checkout — **внутренняя операция** (материализация состояния ветки в хранилище для чтения); REST write-эндпоинты работают с branch snapshots, а не с живой онтологией. |
...
| Код | Функция | Описание |
|---|---|---|
| F6.1 | REST API | Полный набор эндпоинтов для управления онтологиями (аналог основного функционала редактора). Каноническая структура — **project-scoped** (GitLab-выравнивание): операции вложены под `/projects/{pid}/` — repository content (`/projects/{pid}/repository/classes|properties|individuals` с `?ref=`), commits/history, branches, merge_requests, releases, protected_branches. Write-операции — **branch-scoped** (работают с branch snapshots). Пути `/api/v1/ontologies/*` — deprecated (заголовки `Deprecation`/`Sunset`, после окна миграции — `501` с `x-vedo-status: planned`). |
...
| Код | Функция | Описание |
|---|---|---|
| F11.1 | Публичная посадочная страница и витрина демо-онтологий | Общедоступная страница без авторизации: hero с положением о концепции, витрина демо-проектов (VEDO Demos), ролевые разделы для целевой аудитории, призыв к действию (регистрация). Служит точкой входа для новых пользователей. |
| F11.2 | Публичный просмотр | Возможность делиться онтологией с внешними партнёрами и клиентами через публичную ссылку без создания аккаунта. Бинарная модель доступа: **public** (без авторизации) / **restricted** (API key через `X-VEDO-API-Key`). Публикацию выполняет роль Maintainer+ (вес ≥ 2); отдельной роли publisher нет. Visibility снэпшота декоррелирована от visibility Project (приватный проект может публиковать публичный снэпшот). Опубликованные снэпшоты именуются **releases** и экспонируются как `/projects/{pid}/releases`. |
| F11.3 | Быстрая навигация | 2D/3D навигация по структуре онтологии без авторизации. |
| F11.4 | Полнотекстовый поиск | Поиск по элементам опубликованной онтологии. |
...
| F14.5 | Демо-проекты (Demos Group) | 5 демо-проектов для типовых предметных областей (организация, продукт, процесс, глоссарий, событие) в группе `VEDO Demos`. Пользователи форкают демо-проекты через `POST /api/v1/projects/{id}/fork`, получая копию Project + Ontology со ссылкой на upstream. Fork = базовый механизм как для демо-проектов, так и для социального хаба (F13.1). |
...
| Код | Функция | Описание |
|---|---|---|
| F16.1 | CRUD онтологий через API | Полный набор эндпоинтов для создания, чтения, обновления, удаления онтологий, классов, свойств, связей. Канонические пути — project-scoped: `/projects/{pid}/repository/classes|properties|individuals` (с `?ref=`), commits/history, branches. Write-операции — branch-scoped (branch snapshots, write-path invariant). Дополнительные endpoint-классы: `/projects/{pid}/merge_requests` (iid-based, PUT merge, state_event, /diffs), `/projects/{pid}/releases` (published snapshots), `/projects/{pid}/protected_branches`. `ontology_id` — внутренний идентификатор. |
...
flowchart LR
    ...
    F14 --> F14_5["F14.5 Демо-проекты (Demos Group)"]
...
**Цель MVP:** доказать минимально рабочую ценность VEDO Hub как платформы, где любой посетитель может зайти на общедоступную посадочную страницу, познакомиться с демо-онтологиями через наглядный граф связей, зарегистрироваться, и далее: создать проект онтологии в группе, назначить участников и роли, стартовать с демо-проекта (fork) или загруженного документа, получить первичную OWL-структуру, отредактировать TBox/ABox, просмотреть иерархию и полный 2D-граф связей, зафиксировать изменения в истории, увидеть базовый Semantic Diff и работать с онтологией через REST API и MCP-сервер.

**Принцип отсечения scope для MVP:** в MVP входит только то, что необходимо для сквозной демонстрации цепочки:

```text
посадочная страница → группа → проект → участники → демо-проект (fork)/документ → генерация онтологии → редактирование TBox/ABox → просмотр графа → commit/history/diff → REST API/MCP
```
...
1. Посетитель без учётной записи открывает общедоступную посадочную страницу VEDO Hub, просматривает демо-онтологии с наглядным графом связей и регистрируется.
2. `Owner` создаёт группу.
3. `Owner` создаёт проект онтологии внутри группы.
4. `Owner` выбирает способ старта проекта:
   - пустая онтология;
   - демо-проект (fork из `VEDO Demos`);
   - загрузка документа.
5. `Owner` добавляет участников и назначает роли.
6. `Analyst` загружает `DOCX`, `PDF`, `XLSX`, `JSON`, `MD` или `TXT`.
7. VEDO извлекает классы, свойства, связи и базовую иерархию.
8. `Analyst` просматривает результат и подтверждает импорт.
9. Система создаёт или обновляет онтологию и фиксирует initial commit.
10. Пользователь вручную редактирует TBox/ABox.
11. Пользователь просматривает:
    - иерархию классов;
    - TBox-граф;
    - ABox-граф.
12. Пользователь создаёт commit.
13. `Architect` просматривает базовый Semantic Diff между версиями.
14. Внешняя система работает с онтологией через REST API.
15. AI/IDE/LLM-клиент работает с онтологией через MCP-сервер.
...
##### F14. Генерация онтологии из документов и демо-проектов

**Демо-проекты (Demos Group):**
- 5 демо-проектов в группе `VEDO Demos`: Организация, Продукт, Процесс, Глоссарий, Событие.
- Пользователь форкает демо-проект через `POST /api/v1/projects/{id}/fork`, получая копию Project + Ontology со ссылкой на upstream.
- Fork = новый private project в пространстве пользователя. Пользователь становится Owner.
- Fork механизм един для демо-проектов и публичных онтологий (F13.1).
- Демо-проекты создаются как seed data при деплое через `deploy/seeds/vedo-demos/bootstrap.sh`.
- Fork использует существующий `ApplySequence` gRPC (Published Language), не создаёт нового механизма.

**Не входит в MVP (отложено по ADR-DES.PROCESS.templates-via-forks):** merge-upstream-changes (fork → pull updates), каталог демо-проектов как marketplace, версионирование демо-проектов (git tags), пользовательские публичные проекты как демо.
...
- REST API для:
  ...
  - projects CRUD;
  - members/roles CRUD;
  - classes CRUD (`/projects/{pid}/repository/classes` с `?ref=`);
  - object properties CRUD (`/projects/{pid}/repository/properties` с `?ref=`);
  - datatype properties CRUD;
  - annotations CRUD;
  - individuals CRUD (`/projects/{pid}/repository/individuals` с `?ref=`);
  - commits/history (`/projects/{pid}/repository/commits`);
  - Semantic Diff (`/projects/{pid}/repository/commits/{sha}/diff`);
  - read-only SPARQL endpoint;
  - базовый поиск.
- MCP-compatible service layer;
- OpenAPI-документация.

**GitLab-выравнивание:** REST API следует структуре GitLab — операции вложены под `/projects/{pid}/` (см. ADR-DES.API.rest-gitlab-alignment). Write-операции branch-scoped (branch snapshots, write-path invariant). Старые пути `/api/v1/ontologies/*` помечены как deprecated; после окна миграции возвращают `501 Not Implemented` с заголовком `x-vedo-status: planned`. Планируемые эндпоинты (merge_requests, releases, protected_branches) регистрируются как контрактные заглушки с `x-vedo-status: planned`.
...
- publishing;
- полноценный Public Browse 1.0 (публичный поиск, DOI-метаданные, visibility model);
...
| F11 Publishing | Минимально | F11.1: посадочная страница, витрина VEDO Demos; остальное — GUI/API stub |
