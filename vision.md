...
- **Для не-IT-пользователя:** Создание онтологии через описание на естественном языке (LLM генерирует OWL-структуру). Визуальный drag-n-drop конструктор SPARQL-запросов без кода. Загрузка из Excel, использование шаблонов.
...
| **G5** | Снизить порог входа в семантическое моделирование | **S**: Аналитики, учёные и не-IT-пользователи создают онтологии без знания OWL — через описание на естественном языке (LLM генерирует OWL-структуру) и визуальный конструктор SPARQL-запросов drag-n-drop.<br>**M**: Новый пользователь создаёт онтологию из 10 классов через NLP-описание за 5 минут без чтения документации.<br>**A**: LLM-генерация OWL из текста, визуальный SPARQL, загрузка из Excel, шаблоны.<br>**R**: P4 — высокий порог входа для не-экспертов — главное препятствие для роста сообщества онтологий.<br>**T**: К концу 2 кв. | Новый пользователь создаёт онтологию из 10 классов через NL-описание за 5 мин. | 2 кв. | P4 | F14, F15 |
...
> *   **Механизм:** VEDO Hub объединяет (1) социальный хаб (форки, звёзды, PR, DOI), (2) инфраструктурное API (семантический поиск, валидация, SPARQL), (3) LLM-генерацию OWL из текста, (4) визуальный drag-n-drop SPARQL.
...
- Прослеживаемость от Issue до деплоя.

**Уникальное своё:**
1. **LLM-генерация онтологии по тексту** — создание строгой OWL-структуры из описания на естественном языке. Ни одна платформа онтологий этого не умеет.
...
| **2** | **F14: LLM-генерация онтологий (NLP → OWL)** | Создание строгой OWL-онтологии по текстовому описанию на естественном языке. Пользователь описывает предметную область — LLM генерирует классы, свойства, связи, OWL-аксиомы. | Низкий порог входа: онтологию создаёт любой пользователь без знания OWL. |
...
| **F14** | LLM-генерация онтологий (NLP → OWL) | G4, G5 | Генерация OWL-онтологии из текста на естественном языке через LLM: классы, свойства, связи, OWL-аксиомы |
...
| F14.4 | Импорт из Excel → OWL | Загрузка таблицы Excel с колонками (класс, родитель, свойство, тип) → LLM преобразует в OWL-структуру. |
...
flowchart LR
    ...
    F14 --> F14_4["F14.4 Импорт Excel → OWL"]
...
**F1. Просмотр и навигация:**
- Просмотр дерева классов (с возможностью поиска и фильтрации).
- Просмотр списка свойств (Object и Datatype) для выбранного класса.
- Просмотр списка индивидов для выбранного класса.
- Базовая визуализация графа (отображение выбранного класса с его родителями и потомками).

**F2. Редактор онтологий (TBox):**
- CRUD операций для классов (Создание, чтение, редактирование, удаление).
    - Поля: ID (автогенерация), Label, Comment.
    - Установка родительского класса (один или несколько).
- CRUD операций для ObjectProperty.
    - Поля: ID, Label, Comment, Domain (классы), Range (классы).
- CRUD операций для DatatypeProperty.
    - Поля: ID, Label, Comment, Domain (классы), Range (XSD типы: string, integer, date, boolean).
- CRUD операций для аннотаций (пользовательские свойства).
- Иерархическое дерево классов с поддержкой drag-n-drop для изменения иерархии.

**F3. Git-подобное управление версиями (базовое):**
- Создание коммита с сообщением.
- Просмотр истории коммитов.
- Возможность отката онтологии к любому коммиту (восстановление состояния графа).
- Создание и переключение между ветками.
- **ПРИМЕЧАНИЕ:** Блокировки узлов и совместное редактирование в реальном времени не поддерживаются.

**F4. Управление индивидами (ABox):**
- Создание и редактирование экземпляров классов.
- Заполнение значений свойств (ObjectProperty — выбор из списка индивидов; DatatypeProperty — ввод значения).
- Удаление индивидов.

**F13. Социальный хаб (MVP):**
- Публичный профиль автора с базовой информацией и списком онтологий.
- Звёзды для публичных онтологий (отметка «полезно»).
- Базовый механизм форков (копирование публичной онтологии в своё пространство).
- Issues для публичных онтологий с базовыми шаблонами.

**F14. LLM-генерация онтологий (MVP):**
- Базовая NL → OWL генерация: пользователь вводит текстовое описание, LLM генерирует классы и свойства.
- Доработка и уточнение: пользователь может уточнить описание, LLM достраивает структуру.

**F15. Визуальный конструктор SPARQL (MVP):**
- Базовый drag-n-drop построитель SPARQL-запроса: выбор субъекта, свойства, объекта, простые фильтры.
- Выполнение запроса и базовое отображение результатов.

**F16. API как инфраструктурный слой (MVP):**
- CRUD-операции с онтологиями через API (как F6, но дополнено).
- Семантический поиск по всем публичным онтологиям хаба.
- SPARQL-эндпоинт для выполнения запросов.
- Webhooks для уведомлений об изменениях.

**F5. Комментарии и обсуждения (базовое):**
- Добавление комментариев к классам, свойствам, индивидам.
- Отображение комментариев в общем чате проекта.
- Уведомления об ответах на комментарии.

**F8. Импорт/экспорт:**
- Экспорт онтологии в Turtle (.ttl) и RDF/XML.
- Импорт онтологии из Turtle и RDF/XML.

**F6. Интеграция (базовая):**
- REST API для чтения онтологии (получение списка классов, свойств, индивидов).
- Базовая документация API (Swagger/OpenAPI).
- SPARQL endpoint `/api/v1/sparql` и CYPHER endpoint `/api/v1/cypher` для программного выполнения read-only запросов с аутентификацией, авторизацией, защитой от инъекций, timeout, лимитом результатов, rate limiting и аудитом.

**F10. Гибкие запросы к графу знаний:**
- SPARQL-запросы: визуальный конструктор, текстовый редактор с подсветкой синтаксиса и автодополнением, NL-режим (кнопка «Микрофон», опционально, с явным opt-in).
- CYPHER-запросы: визуальный конструктор, текстовый редактор с подсветкой синтаксиса и автодополнением, NL-режим (кнопка «Микрофон», опционально, с явным opt-in).
- MCP-сервер для запросов к графу знаний (внешние LLM-клиенты).
- Introspection онтологии для построения конструкторов и контекста NL-режима.
- Единый формат ответа (таблица или граф) для всех типов запросов с экспортом в CSV, JSON, Turtle.
- Библиотека сохранённых запросов для часто используемых сценариев.

**F9. Администрирование через `vedo-cli` (MVP операций):**
- Ежедневное резервное копирование всей онтологии TBox+ABox по расписанию с сохранением в S3/MinIO-compatible storage и обязательной проверкой `backup verify`.
- Восстановление из backup по RTO-протоколу.
- Применение Neo4j/PostgreSQL миграций с pre-migration backup, идемпотентностью и rollback одной командой.
- Экспорт/импорт онтологий между окружениями, включая canonical Turtle для TBox и `diff` двух версий онтологии.
- Диагностика инцидента по `trace_id`: поиск ошибочного span в Tempo, связанных логов в Loki, метрик в Prometheus и генерация рекомендаций, опционально через LLM.

**F12. Поддержка пользователей и сбор обратной связи (MVP):**
- Портал поддержки — только приём тикетов через форму на сайте + email. Ручная триада и ответы (внутренний процесс). SLA — best effort для Community, письменные SLA для Enterprise.
- In-app виджет «Сообщить о проблеме» с автоматическим приложением метаданных (trace_id, последнее действие).
- In-app NPS — после 10 часов работы или 30 действий пользователя.
- База знаний — статическая документация (Antora) + раздел FAQ.
- Публичная статус-страница в минимальном виде («все системы работают» / «частичный сбой» / «критический сбой») с ручным обновлением.

**Что НЕ входит в MVP по F12:**
- Автоматическая классификация обратной связи (делается вручную).
- Community-форум (запускается на этапе 2).
- Публичный roadmap с голосованием (этап 2).
- Интеграция портала поддержки с PagerDuty для P0/P1 (реализуется на этапе 2).
- Телеметрия использования (opt-in) — на этап 2.

### 2.6. Объем последующих версий (Roadmap)

**Этап 2 (социальный хаб и LLM-генерация):**
...
- Импорт из Excel → OWL через LLM.
- Шаблоны онтологий.
- Сохранение и шеринг SPARQL-запросов.
- Community-форум, публичный roadmap с голосованием.
- Телеметрия использования (opt-in).

**Этап 3 (инфраструктурный слой и монетизация):**
...
**Этап 4 (Enterprise и углубление):**
...
- AI-ассистент: достройка онтологии, подсказки по связям.
...
| 3 | **Поддержка SWRL-правил** в интерфейсе редактора. | Сложность графического конструирования правил. Будет реализовано на этапе 3 как экспорт/импорт текстовых правил. |
...
Ниже — сравнение продуктов по нашим бизнес-целям (G1–G8).
...
| **G5: Низкий порог входа** | △ — веб, но OWL | ✘ — нет | ✘ — нет | ✘ — OWL+десктоп | ✘ — обучение | **★ — NLP→OWL, визуальный SPARQL** |
...
**Итого:** ни один конкурент не закрывает более 3 из 8 целей на уровне ✔ или выше. VEDO Hub — единственный продукт, нацеленный на все 8 бизнес-целей одновременно.
...
| **Бизнес-аналитик** | Создание глоссариев и таксономий без программирования, визуальный SPARQL, загрузка из Excel. | Конечный пользователь | NLP → OWL, визуальный SPARQL, импорт Excel, шаблоны. | Не знает OWL и SPARQL. Нужен NLP-интерфейс. |
