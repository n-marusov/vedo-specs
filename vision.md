...
- Community-форум (запускается на этапе 3).
- Публичный roadmap с голосованием (этап 3).
- Интеграция портала поддержки с PagerDuty для P0/P1 (реализуется на этапе 3).
- Телеметрия использования (opt-in) — на этап 3.
...
- Импорт из Excel → OWL через LLM: загрузка таблицы с колонками (класс, родитель, свойство, тип) — LLM преобразует в OWL-структуру.
